#!/usr/bin/env python3
"""Fetch a proof-verified PRP hit from VM102 and start distinct Mlucas LL."""
...
REMOTE = "root@38.86.78.6"
REMOTE_PRP = "/root/eff-prime/runs/M332228177-prp-20260805T0450Z"
REMOTE_PREDECESSOR = "/root/eff-prime/runs/M332228177-ll-queued/predecessor"
LOCAL = ROOT / "runs/M332228177-mlucas-queued"
SERVICE = "eff-mlucas-m332228177.service"
FILES = (
...
def main() -> int:
    if (LOCAL / "completion-published.txt").exists():
        print("complete: independent Mlucas receipt already published")
        return 0
    marker = ssh(f"test -e {REMOTE_PRP}/PROBABLE_PRIME_TRIGGER_PUBLISHED.txt")
    if marker.returncode != 0:
        print("waiting: proof-verified M332228177 probable-prime trigger is absent")
        return 0
    if (LOCAL / "started_utc.txt").exists():
        ...
        if active.returncode != 0:
            ...
            print("resumed: distinct CPU/Mlucas M332228177 confirmation")
        else:
            print("active: distinct CPU/Mlucas M332228177 confirmation")
        return 0
    ...
    with tempfile.TemporaryDirectory(prefix="eff-mlucas-trigger-", dir=ROOT / "runs") as raw:
        temporary = Path(raw)
        subprocess.run(
        ...
        if transition["exponent"] != 332228177 or transition["classification"] != "probable_prime_trigger":
            raise RuntimeError("remote transition is not an exact probable-prime trigger")
        ...
        if result.get("exponent") != 332228177 or result.get("worktype") != "PRP-3" or result.get("status") != "P":
            raise RuntimeError("remote PRP result is not an exact hit")
        ...
        for name in FILES:
            if digest(temporary / name) != expected[name]:
                raise RuntimeError(f"remote/local transfer mismatch: {name}")
    ...
    subprocess.run(["systemctl", "--user", "start", SERVICE], check=True)
    print("started: distinct CPU/Mlucas M332228177 Lucas-Lehmer confirmation")
    return 0
