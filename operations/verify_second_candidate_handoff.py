...
def main() -> int:
    ...
    # handoff upgrades and the third reservation. Preserve its historical
    # hashes, then bind the current deployment through the authoritative v5
    # receipt.
    ...
    initial_ledger = next(
        value for path, value in hashes.items()
        if path.endswith("M332228213-pminus1-queued/ledger/mersenne_candidates.jsonl")
    )
    require(
        initial_ledger == "878698039b02ec399c66ce0dd5cd9a61152240220d6f6fa44d463a83c4d70fa3",
        "initial ledger history",
    )
    v5 = json.loads((allocation_root / "vm101-full-pipeline-staging-v5.json").read_text())
    current_handoff = v5["staged_hashes"][
        "/root/eff-prime/runs/M332228213-pminus1-queued/handoff.sh"
    ]
    require(
        current_handoff == digest(ROOT / "scripts/handoff_p40_to_M332228213.sh"),
        "current v5 handoff binding",
    )
    current_ledger = v5["staged_hashes"][
        "/root/eff-prime/runs/M332228213-pminus1-queued/ledger/mersenne_candidates.jsonl"
    ]
    require(
        current_ledger == digest(ROOT / "ledger/mersenne_candidates.jsonl"),
        "current v5 ledger binding",
    )
