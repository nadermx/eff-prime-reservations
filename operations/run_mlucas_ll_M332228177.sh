#!/bin/bash
# Distinct CPU/Mlucas LL confirmation admitted only by a proof-verified PRP hit.
...
expected_directory="/home/john/eff/runs/M332228177-mlucas-queued"
...
expected_config_sha256="e22efd897f83185b72694cfafcf4b259e1e3d1584b454b988ed12fdd4ce6e398"
...
fail() { echo "refusing Mlucas confirmation launch: $*" >&2; exit 64; }
...
[ "$(sha256sum mlucas.cfg | awk '{print $1}')" = "$expected_config_sha256" ] || fail "validated radix configuration mismatch"
...
if python3 verify_mlucas_result.py p332228177.stat \
    --exponent "$expected_exponent" > validated-mlucas-result.json 2>/dev/null; then
    echo "validated terminal Mlucas result already exists; refusing duplicate work"
    exit 0
fi
...
if pgrep -f '[M]lucas.*332228177|[M]lucas-main-4a21413-avx2-safe' >/dev/null; then fail "Mlucas target process already active"; fi

if [ "$mode" = --preflight-only ]; then
    echo "PASS: proof-verified trigger, distinct CPU binary/source, radix, and disk gates"
    exit 0
fi
...
python3 verify_mlucas_result.py p332228177.stat \
    --exponent "$expected_exponent" > validated-mlucas-result.json || {
        echo "Mlucas exited zero without an admissible exact LL result" >&2
        exit 75
    }
...
receipt = {
    ...
    "stat_file_sha256": digest("p332228177.stat"),
    ...
    "h200_used": False,
    "scope": "Independent deterministic Lucas-Lehmer confirmation; publication and EFF claim remain separate gates.",
}
...
PY
sha256sum completion-receipt.json validated-mlucas-result.json p332228177.stat mlucas.stdout mlucas.time > completion-provenance.sha256
