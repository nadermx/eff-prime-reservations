#!/bin/bash
# Classify the completed PRP, verify its proof, and admit LL only after a hit.

set -euo pipefail

root="/root/eff-prime"
prp="$root/runs/M332228177-prp-20260805T0450Z"
ll="$root/runs/M332228177-ll-queued"
...
fail() { echo "PRP-to-LL handoff refused: $*" >&2; exit 75; }
...
if [ -e "$prp/PROBABLE_PRIME_TRIGGER_PUBLISHED.txt" ]; then
    if [ ! -e "$ll/completion-published.txt" ] && \
       [ "$(systemctl is-active eff-ll-m332228177.service)" = inactive ]; then
        systemctl start eff-ll-m332228177.service
    fi
    exit 0
fi

[ -e "$prp/completed_utc.txt" ] || fail "PRP completion marker absent"
[ "$(systemctl is-active eff-prp-m332228177.service)" = inactive ] || fail "PRP service has not exited"
...
receipt = {
    ...
    "result": "PASS",
    "scope": "PRP execution-proof verification; not a deterministic primality proof.",
}
